import sys
from rare_event import iMPS, MarkovianRecurrentQuantumCircuit, ShiftedDiagonalBlockEncoding, QET, plot_uniform, plot_importance
import numpy as np
...
func = lambda x: -erf(delta*x)
max_scale = 0.9 # Maximum norm (<1) for rescaling.
true_func = lambda x: -np.sign(x)
...
if SIMULATE:
    dev2 = qml.device("lightning.qubit", wires = base_qubits + 5)
    all_wires = list(range(base_qubits + 4))
    wires = list(range(4, base_qubits + 4))
    main_wires = list(range(5, base_qubits + 4))
    other_wires = [0, 1, 2, 3, 4]
    ancilla_wires = [2, 3]
    control_wires = [2, 3, 4]
    rotation_wire = 1
    lcu_wire = 0
    work_wire = base_qubits + 4
else: 
...
original = circuit()
thresholded = np.where(pow(original, 0.5) < threshold, 1, 0)
